...
\headerbox{Introduction}{name=intro,column=0,row=0,span=2,boxColorOne=white}{
    ...
    They can form quark-antiquark states called quarkonium - these can be studied similarly to the Hydrogen atom. 
    The charm-anticharm state is known as Charmonium, and this will be the focus of this study.  
}

\headerbox{Theory}{name=theory,below=intro,column=0, span=1,boxColorOne=white}{
    The Charmonium wavefunction comes through solving the 3D Schrodinger equation \cite{qnl},
    \vspace{-10pt}
    \begin{align}
...
\headerbox{The Bisection Method}{name=bis,below=theory,above=bottom,column=0,span=1,boxColorOne=white}{
    This method guess three energies, $E_1, E_3$, and $E_2 = \frac{E_1+E_3}{2}$ to find $u_{nl}$.
...
\headerbox{The Hydrogen Wavefunction}{name=hydro,column=1,below=intro,span=1,boxColorOne=white}{
    To check the program is functioning for a simpler model, we apply it to the Hydrogen electron.
...
\headerbox{Calculating $\beta$}{name=beta,column=1,above=bottom,below=hydro,span=1,boxColorOne=white}{
   ...
   \vspace{-5pt}
    \begin{equation}
        M_{nl} = E_{nl} + 2m_c,~ M_{nl} = 3.068\text{Gev}/c^2,~ m_c = 1.34\text{GeV}/c^2
    \end{equation}
...
\headerbox{Finding the Charmonium Wavefunctions}{name=comp,above=references,column=2,span=1,boxColorOne=white}{
    With a known value of $\beta = 0.195$, we can proceed to solve the Charmonium wavefunction, with
    \begin{equation}
        \alpha_s = 0.4.
    \end{equation}
    ...
    Now using the same method as before, having confirmed the functionality of the program on hydrgoen, the solutions of the radial wavefunction are solved.
...
\headerbox{Outlook}{name=outlook,column=3,span=1,boxColorOne=white}{
    Now with a functioning program, further extensions to be applied to the study are considered. 
    The energies of the states were seen to be somewhat innacurate to the known measurements, at least partially due to the hyperfine spin-spin interaction causing a splitting in the energy.
    This splitting is
    \vspace{-10pt}
    \begin{equation}
        \Delta(n^3S_1 - n_1S_0) = \frac{8\alpha_s}{9m_q^2}|R_{nl}(0)|^2,
    \end{equation}
    so the energies calculated in this project will be the average of the two hyperfine energies, and these values can be found using this splitting formula.
    ...
    \end{equation}
    Further from this, the transition rates between the higher and lower hyperfine energies can be stuided.
}
